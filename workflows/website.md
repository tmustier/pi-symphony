...
hooks:
  after_create: |
    git clone --depth 1 git@github.com:Nexcade/website.git .
worker:
  runtime: pi
agent:
...
rollout:
  ...
  kill_switch_file: /tmp/pi-symphony-website.pause
pr:
  auto_create: true
  base_branch: main
  repo_slug: Nexcade/website
...
You are working on a Linear ticket `{{ issue.identifier }}` in the **Nexcade website** repository.
...
5. Validate: run the project's build/lint/test commands.
