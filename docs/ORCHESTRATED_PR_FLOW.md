...
Passive polling should use the PR-inspection helper to refresh live gates (`checks`, `human_approval`, `mergeability`, `head_match`) and may safely promote `waiting_for_checks` into `waiting_for_human` or `ready_to_merge` without reopening implementation work.
