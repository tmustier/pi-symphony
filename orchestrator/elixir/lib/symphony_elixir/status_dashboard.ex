defmodule SymphonyElixir.StatusDashboard do
  ...
  defp snapshot_with_samples(token_samples, now_ms) do
    case snapshot_payload() do
      {:ok, %{running: running, retrying: retrying, worker_totals: worker_totals} = snapshot} ->
        total_tokens = Map.get(worker_totals, :total_tokens, 0)
  ...
  defp format_snapshot_content(snapshot_data, tps, terminal_columns_override \\ nil) do
    case snapshot_data do
      {:ok, %{running: running, retrying: retrying, worker_totals: worker_totals} = snapshot} ->
        ...
        worker_input_tokens = Map.get(worker_totals, :input_tokens, 0)
        worker_output_tokens = Map.get(worker_totals, :output_tokens, 0)
        worker_total_tokens = Map.get(worker_totals, :total_tokens, 0)
        worker_seconds_running = Map.get(worker_totals, :seconds_running, 0)
        ...
        ([
           ...
           colorize("│ Runtime: ", @ansi_bold) <>
             colorize(format_runtime_seconds(worker_seconds_running), @ansi_magenta),
           colorize("│ Tokens: ", @ansi_bold) <>
             colorize("in #{format_count(worker_input_tokens)}", @ansi_yellow) <>
             colorize(" | ", @ansi_gray) <>
             colorize("out #{format_count(worker_output_tokens)}", @ansi_yellow) <>
             colorize(" | ", @ansi_gray) <>
  ...
  defp format_running_summary(running_entry, running_event_width) do
    ...
    pid = format_cell(running_entry.worker_pid || "n/a", @running_pid_width)
    total_tokens = running_entry.worker_total_tokens || 0
    ...
    event = running_entry.last_worker_event || "none"
    ...
    event_label = format_cell(summarize_message(running_entry.last_worker_message), adjusted_event_width)
  ...
  defp snapshot_total_tokens({:ok, %{worker_totals: worker_totals}}) when is_map(worker_totals) do
    Map.get(worker_totals, :total_tokens, 0)
  end
